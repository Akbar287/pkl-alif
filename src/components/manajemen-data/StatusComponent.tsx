...
import {
  ...
  setStatus,
  updateStatus,
} from "@/services/ManajemenData/StatusServices";
...
const StatusComponent = () => {
  ...
  const form = useForm<StatusFormValidation>({
    resolver: zodResolver(StatusSkemaValidation),
    defaultValues: {
      Nama: "",
    },
  });
  const onSubmit = async (data: StatusFormValidation) => {
    setLoading(true);

    if (titleDialog === "Ubah Status") {
      await updateStatus({
        StatusId: data.StatusId,
        Nama: data.Nama,
      })
        .then((res) => {
          toast("Data Status berhasil diubah");
          let idx = dataStatus.findIndex((r) => r.StatusId === data.StatusId);
          setDataStatus(replaceItemAtIndex(dataStatus, idx, res));
          setOpenDialog(false);
          setLoading(false);
        })
        .catch((err) => {
          toast("Data Status gagal diubah. Error: " + err);
          setLoading(false);
        });
    } else {
      await setStatus({
        ...
        .then((res) => {
          toast("Data Status berhasil ditambah");
          setDataStatus([...dataStatus, res]);
          setLoading(false);
          setOpenDialog(false);
        })
        .catch((err) => {
          toast("Data Status gagal ditambah. Error: " + err);
          setLoading(false);
        });
    }
  };
  ...
  React.useEffect(() => {
    setLoading(true);
    getStatusPagination(paginationState.page, paginationState.limit, search)
      .then((res) => {
  ...
  const ubahData = (jd: Status) => {
    form.setValue("StatusId", String(jd.StatusId));
    form.setValue("Nama", String(jd.Nama));
    setTitleDialog("Ubah Status");
    setOpenDialog(true);
  };
  // const hapusData = (jd: Status) => {
  //   Swal.fire({
  ...
  //       deleteStatus(jd.StatusId).then(() => {
  //         setDataStatus(dataStatus.filter((r) => r.StatusId !== jd.StatusId));
  //         Swal.fire({
  //           title: "Terhapus!",
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem
                onClick={() => navigator.clipboard.writeText(jd.StatusId)}
              >
                Copy Status ID
              </DropdownMenuItem>
              <DropdownMenuSeparator />
...
export function SheetManageData({
  ...
  loading: boolean;
  onSubmit: (data: StatusFormValidation) => void;
  form: UseFormReturn<StatusFormValidation>;
  titleDialog: string;
}) {
  return (
