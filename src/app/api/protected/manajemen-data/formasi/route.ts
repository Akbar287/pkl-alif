...
app.get('/', async (c) => {
    ...
    if (id) {
        ...
        let where: Prisma.FormasiWhereInput = search
            ? magangId ? {
                  AND: [{ Nama: { contains: search, mode: 'insensitive' } }, {Magang: {Nama: {contains: search, mode: 'insensitive'}}}, {MagangId: magangId}],
              }
            :
            {
                  AND: [{ Nama: { contains: search, mode: 'insensitive' } }, {Magang: {Nama: {contains: search, mode: 'insensitive'}}}],
              }
            : {}
        ...
        data = magangId ? await prisma.formasi.findMany({where: {MagangId: magangId}}) : await prisma.formasi.findMany()
    }
